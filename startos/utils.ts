...
// Runtime boot chatter; it shares stdout with the real output.
const VIKUNJA_BOOTSTRAP_LOG =
  /^(No config file found|Running migrations|Using SQLite|Ran all migrations|No license key)/

// `repair` reports as log lines, so `stripVikunjaLogs` would drop its whole output.
export function unwrapVikunjaLogs(text: string): string {
  return text
    .split('\n')
    .map((line) => {
      // \r is a progress bar redrawing in place; keep the final frame.
    ...
    .filter(({ msg }) => msg.trim() && !VIKUNJA_BOOTSTRAP_LOG.test(msg.trim()))
    .map(({ level, msg }) => (level === 'INFO' ? msg : `${level}: ${msg}`))
    .join('\n')
    .trim()
}

// Vikunja's `log.Fatalf` writes its `level=ERROR` line to stdout, not stderr.
export function cliFailure(res: {
  exitCode: number | null
  stdout: { toString(): string }
  stderr: { toString(): string }
}): string {
  const lines = unwrapVikunjaLogs(
    `${res.stderr.toString()}\n${res.stdout.toString()}`,
  )
    .split('\n')
    .filter(Boolean)
  const errors = lines
    ...
    .map((l) => l.replace(/^(ERROR|FATAL): /, ''))
  return (
    (errors.length ? errors : lines).join('\n') || `exit code ${res.exitCode}`
  )
}
